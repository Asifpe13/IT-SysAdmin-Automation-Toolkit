...
# נסה לייבא את WMI. אם נכשל (למשל בלינוקס/מק), נרוץ במצב סימולציה
try:
    import wmi
    WMI_AVAILABLE = True
except ImportError:
...
# הגדרות סף (Thresholds)
MIN_RAM_GB = 8
MIN_DISK_FREE_GB = 20

# רשימת מחשבים לסריקה
# בבית נבדוק את localhost. בעבודה אמיתית הרשימה תגיע מ-AD.
...
def get_remote_info(computer_name):
    ...
    # --- SIMULATION FOR PORTFOLIO ---
    # כאן אנחנו מדמים מה היה קורה ברשת אמיתית עבור מחשבים שאין לנו גישה אליהם בבית
